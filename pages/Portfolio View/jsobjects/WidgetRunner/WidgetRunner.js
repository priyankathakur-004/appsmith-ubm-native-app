export default {
	...
	runAll() {
		setTimeout(() => {
			const widgets = fetchWidgets.data || [];

			widgets.forEach(widget => {
				const queryName = this.QUERY_MAP[widget.name];
				if (!queryName) return console.warn("Missing query:", widget.name);
		...
		payload.filters.forEach(f => {
			switch (f.filter) {
				...
				case "time_period":
					...
					const tp = f.time_period;
					this.applyTimePeriod(filters, tp);
					break;
			}
		});
